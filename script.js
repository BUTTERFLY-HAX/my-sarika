...
const PARAGRAPH_TEXT = `അആഇഈഉഊഋഎഏഐഒഓഔഅംഅഃകഖഗഘങചഛജഝഞടഠഡഢണതഥദധനപഫബഭമയരലവശഷസഹളഴറൺൻൽർൾAi.`;
...
// helpers
const seg = new Intl.Segmenter("ml", { granularity: "grapheme" });
...
function init() {
  ...
  // reset state
  gIndex = 0; cpIndex = 0; mistakes = 0; typedEcho = "";
  started = false; startTimeMs = 0;
...
function finish() {
  if (timerId) { clearInterval(timerId); clearInterval(timerId); timerId = null; }
...
function acceptCodepoint(cp) {
  ...
  if (cpIndex >= graphemeCPs[gIndex].length) {
    ...
    if (nextSpan) nextSpan.scrollIntoView({block:"nearest", inline:"nearest"});
  }
  updateStatsUI();
...
function flashMistake() {
  ...
  setTimeout(() => { scroller.style.outline = "none"; flashing = false; }, 120);
}
